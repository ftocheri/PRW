...
window.onload = function () {
    var data = JSON.parse(this.localStorage.getItem('article'));
    $.each(data, function (i, item) {
        var myList = $('#myList');
        imgNum = Math.floor(Math.random() * 9);
        img = '<img src="images/' + imgNum + '.jpg" alt="Recipe Image">';

        $("#myList").append("<article id=" + item.id + "><h4>" + img + item.name + "</h4><p>" + item.desc + "</p><p>Category: " + item.cat + "</p><p>Star Rating: " + item.rate + "</p><p class='remove' onclick='deleteMe(this);'>" + "<span class='fa fa-trash'></span>" + "</p></article>");
    });
};
...
fetch('https://api.myjson.com/bins/gg3eh').then(function (response) {
    ...
    $.each(data.recipes, function () {
        $("#myList").append("<article><h4>" + img + this.title + "</h4><p>" + this.description + "</p><p>Category: " + this.category + "</p><p>Star Rating: " + this.starRating + "</p><p class='remove' onclick='deleteMe(this);'>" + "<span class='fa fa-trash'></span>" + "</p></article>");
    });
...
function myList() {
    ...
    $("#myList").append("<article id=" + arrayLength + "><h4>" + img + $("#rName").val() + "</h4><p>" + $("#rDesc").val() + "</p><p>Category: " + $("#rCat").val() + "</p><p>Star Rating: " + r + "</p><p class='remove' onclick='deleteMe(this);'>" + "<span class='fa fa-trash'></span>" + "</p></article>");
